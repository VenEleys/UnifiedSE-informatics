class Utils:
    ...
    def tenToSystem(num: int, system: int, returnStr = True) -> str:
        ...
        for i in Utils.letters:
            if i in ans:
                raise ValueError(f"can`t convert number with base {system} containing literals to int")
        return int(ans)
    ...
    def systemToTen(num: str, system: int, returnStr: bool = False) -> str:
        ...
        for index, i in enumerate(num):
            if i >= system:
                raise ValueError(f"invalid number with base {system}: containing a number ({i}) larger or equal base")
